...
// Chart 1: 首購時間分布 (Bar + cumulative line)
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function NewMemberAnalysis() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      ...
      {/* Header */}
      <div className="mb-4">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">新會員分析</h1>
        <p className="mt-1 text-sm text-slate-500">
          分析新客來源品質、首購速度與轉換深度，協助優化新會員引導策略。
        </p>
      </div>
      ...
      {/* ── Chart 1: 首購時間分布 Combo ── */}
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
        <h3 className="text-base font-bold text-slate-800 mb-1">⏱️ 新會員首購時間分布</h3>
        <p className="text-xs text-slate-400 mb-5">
          各區間首購比例（柱）與累計首購率（折線）—— 點擊區間查看該批會員名單
        </p>
        <div style={{ height: 320 }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              />
              <Bar yAxisId="left" dataKey="count" name="首購人數" radius={[6, 6, 0, 0]} cursor="pointer">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                ...
                onClick={(s: any) => {
                  ...
                  if (name) openDrawer(`首購「${name}」會員名單`);
                }}
              >
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
          ...
          <div style={{ height: 260 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                ...
                onClick={(s: any) => {
                  const name = s?.activePayload?.[0]?.payload?.name;
                  if (name) openDrawer(`${name} 渠道新會員名單`);
                }}
              >
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#475569', fontSize: 12, fontWeight: 600 }} />
                ...
                />
                <Bar dataKey="count" name="新會員數" radius={[6, 6, 0, 0]} cursor="pointer">
